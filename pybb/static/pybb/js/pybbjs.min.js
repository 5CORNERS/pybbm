...
$(".tooltip-who-liked-container .fa-chevron-right").on("click",function(){var a=$(this).parent();a.get(0).scrollTo(a.scrollLeft()+a.width(),0)});$(".tooltip-who-liked-container .fa-chevron-left").on("click",function(){var a=$(this).parent();a.get(0).scrollTo(a.scrollLeft()+-a.width(),0)});var tooltipShowTimer,tooltipHideTimer,delay;delay=touch?0:500;
$(".like-container").mouseover(function(){if($(this).hasClass("show-tooltip"))clearTimeout(tooltipHideTimer);else{var a=$(this);tooltipShowTimer=setTimeout(function(b){b=void 0===b?a:b;b.addClass("show-tooltip")},delay)}});$(".like-container").mouseout(function(){if($(this).hasClass("show-tooltip")){var a=$(this);tooltipHideTimer=setTimeout(function(b){b=void 0===b?a:b;b.removeClass("show-tooltip")},delay)}else clearTimeout(tooltipShowTimer)});
...
if(0<c.length){var f=function(){if(this.value){a(this).unbind("change",f);a(this).closest(".attachment-item").find(".attachment-ref").toggle();var b=parseInt(a("#id_attachments-TOTAL_FORMS").attr("value"));if(b<parseInt(a("#id_attachments-MAX_NUM_FORMS").attr("value"))){var e=c.find(".attachment-list");var g=e.find(".attachment-item").last().clone(!0);g.find("#id_attachments-"+(b-1)+"-id").attr("id","id_attachments-"+b+"-id").attr("name","id_attachments-"+b+"-id");g.find("#id_attachments-"+(b-1)+
"-file").replaceWith('<input id="id_attachments-'+b+'-file" type="file" name="attachments-'+b+'-file" />');g.find(".attachment-ref-value").html("[file-"+(b+1)+"]");e.append(g);g.find(".attachment-ref").toggle();a("#id_attachments-TOTAL_FORMS").attr("value",b+1);current_input_file=a("#id_attachments-"+b+"-file");current_input_file.on("change",f)}}};c.find(".attachment-list .attachment-item INPUT[type=hidden]").attr("value")?(c.find(".attachment-list").show(),c.find(".attachment-link").hide()):(c.find(".attachment-list").hide(),
c.find(".attachment-link").show());c.find(".attachment-ref").each(function(b){a(this).closest(".attachment-item").find("INPUT[type=hidden]").attr("value")?a(this).show():a(this).hide()});c.find(".attachment-ref-insert-link, .attachment-ref-insert-image").show().css("cursor","pointer");c.find(".attachment-link").on("click",function(b){b.preventDefault();c.find(".attachment-link").toggle();a(this).closest(".attachments-form").find(".attachment-list").toggle()});c.find(".attachment-ref-insert-link").on("click",
function(b){b.preventDefault();var c=a(this).closest(".attachment-item");b=c.find(".attachment-ref-value").text();c=c.find("INPUT[type=file]").get(0);var d="";c&&c.files&&c.files[0]?d=c.files[0].name:c&&c.value&&(d=c.value.split("\\"),d=d[d.length-1].split("/"),d=d[d.length-1]);window.pybb.insert_link(b,d)});c.find(".attachment-ref-insert-image").on("click",function(b){b.preventDefault();b=a(this.parentNode).find(".attachment-ref-value").text();window.pybb.insert_image(b)});c.find("INPUT[type=file]").last().on("change",
